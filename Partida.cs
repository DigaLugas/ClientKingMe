...
namespace ClientKingMe
{
    public partial class Partida : Form
    {
        ...
        {
            string retorno = Jogo.ListarCartas(
                Convert.ToInt32(ValoresJogo["idJogador"]),
                ValoresJogo["senhaJogador"]
            );
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selecione um personagem e um andar", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            try
            {
                var fase = Jogo.VerificarVez(Convert.ToInt32(ValoresJogo["idPartida"]));
                var linhas = fase.Split(new[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);

                if (linhas.Length == 0)
                {
                    MessageBox.Show("Não foi possível obter o estado atual do jogo.", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }

                var dados = linhas[0].Split(',');
                ...
                {
                    MessageBox.Show("Nenhuma ação válida foi encontrada.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            ...
            {
                MessageBox.Show(
                    $"Erro ao processar movimento: {ex.Message}",
                    "Erro",
